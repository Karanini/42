basic_mem_tests_only_bonus() {
	...
	local TEST_NAME="Basic mem tests"
	test_box "Basic mem tests (checker)"
	...
	PG_PARAMS=()
	PG_PARAMS+=("$CHECKER" 2 "mem" "err" "fail" '^$' '^$')

	core_test

	PG_PARAMS=()
	PG_PARAMS+=("$CHECKER" 2 "mem" "err" "fail" '^$' '^Error$')
	...
	PG_PARAMS=()
	PG_PARAMS+=("$CHECKER" 2 "mem" "err" "fail" '^KO$' '^$')
